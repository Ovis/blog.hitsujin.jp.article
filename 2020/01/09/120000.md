...
[f:id:Ovis:20190831230329p:plain]
...
[f:id:Ovis:20190831232228p:plain]
...
[f:id:Ovis:20190901123806p:plain]
...
[f:id:Ovis:20190901125301p:plain]  
1を選択。  

[f:id:Ovis:20190901125332p:plain]  
1を選択。  

[f:id:Ovis:20190901125401p:plain]  
1を選択。  
...
[f:id:Ovis:20190901134422p:plain]

初回接続時はパスワードを指定しろとウィンドウが出るので入力。  

簡易セットアップウィザードが表示されるので、 `リモートアクセスVPNサーバー` にチェックを入れて `次へ` ボタンをクリック。  
[f:id:Ovis:20190901134544p:plain]

確認ダイアログが表示されるので、`はい` をクリック。  
[f:id:Ovis:20190901134633p:plain]

仮想Hub名は何でもよいのでそのままにしておく。  
[f:id:Ovis:20190901134726p:plain]

##### ダイナミックDNS機能の設定  
ホスト名をお好みのものに変更。  
[f:id:Ovis:20190901134831p:plain]
...
[f:id:Ovis:20190901134910p:plain]

##### VPNAzureサービスの設定  
お好みで。  
[f:id:Ovis:20190901135124p:plain]

##### VPN接続ユーザー作成 

`ユーザーを作成する` ボタンをクリック。  
[f:id:Ovis:20190901135216p:plain]

ユーザー名と認証方法を設定。今回はとりあえずパスワード認証で。  
[f:id:Ovis:20190901135316p:plain]

##### ローカルブリッジの設定  
ローカルブリッジの設定はとりあえずeth0を選ぶ。  
[f:id:Ovis:20190901154148p:plain]

`ローカルブリッジ設定` ボタンをクリック。  
[f:id:Ovis:20190901135548p:plain]

既存のローカルブリッジを一度削除。(よく考えたら上の作業では何もする必要がなかったかも)  
[f:id:Ovis:20190901135641p:plain]

新しいローカルブリッジの定義を作成する。  
`新しいtapデバイスとのブリッジ接続` を選択し、 `新しいtapデバイス` に適当な名前を入れて、  `ローカルブリッジを追加` をクリック。  
[f:id:Ovis:20190901135920p:plain]
